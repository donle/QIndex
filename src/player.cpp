...
Player::Player(QObject *parent)
    ...
    face_up(true), chained(false), clear_temp(true)
{
}
...
bool Player::canSlash(const Player *other, bool distance_limit) const{
    ...
    int attack_range = getAttackRange();
...
void Player::setTempData(const Player *key, int value, bool clear_at_notactive){
    if(!clear_at_notactive)
        clear_temp = false;

        this->temp_data[key] << value;
}

QList<int> Player::getTempData(const Player *key) const{
    return this->temp_data[key];
}

void Player::clearTempData(bool is_clear){
    if(is_clear)
        clear_temp = true;

    if(clear_temp && !temp_data.isEmpty())
