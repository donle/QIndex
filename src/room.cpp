...
void Room::obtainCard(ServerPlayer *target, const Card *card, bool open){
    if(card == NULL)
        return;
    ...
    if(card->isVirtualCard()){
        ...
        obtainCard(target, card->getId(), open);
}

void Room::obtainCard(ServerPlayer *target, int card_id, bool open){
    moveCardTo(Sanguosha->getCard(card_id), target, Player::Hand, open);
}
...
int Room::getDrawpileNumber() const{
    return draw_pile->length();
}
...
void Room::doGuanxing(ServerPlayer *zhuge, const QList<int> &cards, bool up_only, bool no_move){
